...
#pragma warning disable CS3009
    ...
    /// <summary>
    /// Net wrapper around native hotkey implementation.
    /// </summary>
    public class HotkeySupport : IDisposable {

        #region readability constants

        private const int CONTROLLENGTH = 7;
        private const int WINALTLENGTH = 3;
        private const int SHIFTLENGTH = 5;
        private const int SHORTESTIDLENGTH = 3;

        #endregion

        private List<HotkeyStore> interestedKeyCombos = new List<HotkeyStore>();
        private int incrementalHotkeyRegister;
        private bool disposed;

        public HotkeySupport() {
        }
        ...
        private VK_Keys ParseKeyPartOfString(string keyString) {
            keyString = keyString.ToUpperInvariant();
            VK_Keys result = (VK_Keys)0;

            foreach (var v in Enum.GetNames(typeof(VK_Keys))) {
                if (v.Equals(keyString, StringComparison.OrdinalIgnoreCase)) {
                    result = (VK_Keys)Enum.Parse(typeof(VK_Keys), keyString, true);
                    break;
                }
            }
            ...
            if (result == (VK_Keys)0) {
                ...
                #region switch statement that checks all of the keys that have friendly names

                switch (keyString) {
                    case "NUM+": result = VK_Keys.Oemplus; break;
                    case "NUM0": result = VK_Keys.NumPad0; break;
                    case "NUM1": result = VK_Keys.NumPad1; break;
                    case "NUM2": result = VK_Keys.NumPad2; break;
                    case "NUM3": result = VK_Keys.NumPad3; break;
                    case "NUM4": result = VK_Keys.NumPad4; break;
                    case "NUM5": result = VK_Keys.NumPad5; break;
                    case "NUM6": result = VK_Keys.NumPad6; break;
                    case "NUM7": result = VK_Keys.NumPad7; break;
                    case "NUM8": result = VK_Keys.NumPad8; break;
                    case "NUM9": result = VK_Keys.NumPad9; break;
                    case "1": result = VK_Keys.D1; break;
                    case "2": result = VK_Keys.D2; break;
                    case "3": result = VK_Keys.D3; break;
                    case "4": result = VK_Keys.D4; break;
                    case "5": result = VK_Keys.D5; break;
                    case "6": result = VK_Keys.D6; break;
                    case "7": result = VK_Keys.D7; break;
                    case "8": result = VK_Keys.D8; break;
                    case "9": result = VK_Keys.D9; break;
                    case "0": result = VK_Keys.D0; break;

                    default: throw new ArgumentOutOfRangeException("keyString", "The key value was not of the expected type. Not all keys are registered. (" + keyString + ")");
                }
        ...
        public bool ProcessWindowsMessage(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam) {
            if (interestedKeyCombos.Count == 0) { return false; }

            if (msg == (int)NativeMethods.WindowMessages.HOTKEY) {
                VK_Keys key = (VK_Keys)(((int)lParam >> 16) & 0xFFFF);
                MK modifier = (MK)((int)lParam & 0xFFFF);

                foreach (var hitTest in interestedKeyCombos) {
                    if ((hitTest.Modifier == modifier) && (hitTest.Controller == key)) {
                        hitTest.Action(hitTest.ActionParameter);
                        return true;
                    }
                }
            }
        ...
        public bool ProcessWindowsMessage(Message m) {
            return ProcessWindowsMessage(m.HWnd, m.Msg, m.WParam, m.LParam);
        }
        ...
        private void UnregisterAllHotkeys() {
            foreach (var hks in interestedKeyCombos) {
                hks.Unregister();
            }
        }
